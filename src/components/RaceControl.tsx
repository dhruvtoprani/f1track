import {
  Activity, ArrowLeft, ArrowRight, BarChart3, CloudSun, Download, FlaskConical,
  Gauge, LockKeyhole, RotateCcw, ShieldCheck, Sparkles, Timer, Trophy,
...
export function ForecastRunner({ circuit, track, grid, weather, total, progress }: RunnerProps) {
  ...
  return (
    <main className="race-page forecast-page" aria-live="polite">
      <section className="forecast-command-bar">
        <div className="race-identity">
          <span className={`live-pill ${sampleComplete ? 'is-complete' : ''}`}><i /> {sampleComplete ? 'SIMULATION COMPLETE' : 'MODEL RUNNING'}</span>
      ...
      <section className="runner-stage">
        <div className="runner-map">
          ...
          <div className="runner-progress-shell">
            ...
            <div className="runner-progress"><i style={{ width: `${progress?.progress ? progress.progress * 100 : 0}%` }} /></div>
...
export function RaceControl({ simulation, previousSimulation, onBack, onResample }: RaceControlProps) {
  ...
  return (
    <main className="race-page forecast-page">
      <section className="race-command-bar forecast-results-bar">
        ...
        <div className="race-actions"><button className="secondary-button" onClick={exportForecast}><Download size={15} /> Export</button><button className="resample-button" onClick={onResample} title="Keep the circuit, weather, model and qualifying grid fixed; draw 10,000 new race outcomes"><RotateCcw size={15} /> Resample 10,000</button></div>
      </section>
      ...
      <section className="forecast-hero forecast-hero-refined">
        <div className="forecast-leader-card" style={{ '--team-color': winnerTeam.color } as CSSProperties}>
          ...
          <p>Most frequent winner across the full forecast. This is a probability estimate—not a single selected race.</p>
        </div>
      ...
      <section className={`resample-explainer ${previousSimulation ? 'has-comparison' : ''}`}>
        ...
        <article><Waves size={18} /><div><small>RESAMPLED</small><strong>10,000 new race worlds</strong><p>Incidents, reliability, strategy, traffic and execution.</p></div></article>
      ...
      <section className="debrief-section forecast-analysis">
        <div className="debrief-heading"><div><span className="eyebrow"><FlaskConical size={14} /> Outcome intelligence</span><h2>10,000-race forecast</h2></div><div className="debrief-tabs" role="tablist">
          <button className={tab === 'forecast' ? 'is-active' : ''} onClick={() => setTab('forecast')}><BarChart3 size={13} /> Win forecast</button>
          <button className={tab === 'strategy' ? 'is-active' : ''} onClick={() => setTab('strategy')}><Timer size={13} /> Strategy</button>
          <button className={tab === 'model' ? 'is-active' : ''} onClick={() => setTab('model')}><Sparkles size={13} /> Model</button>
        </div></div>
        ...
        {tab === 'forecast' && (
          <div className="forecast-board">
            ...
            <aside className="forecast-method-card">
              ...
              <p>Each percentage is the share of 10,000 modeled worlds won by that driver. “Resample” keeps the forecast inputs fixed and draws a new set of worlds, so small movements are expected.</p>
        ...
        {tab === 'strategy' && <div className="strategy-board forecast-strategies">{simulation.monteCarlo.slice(0, 12).map((entry, index) => {
          const driver = driverById.get(entry.driverId)!
          const team = teamById.get(driver.teamId)!
        ...
        {tab === 'model' && <div className="model-card-layout">
