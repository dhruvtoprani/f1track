...
| Shareable forecast receipt with result, reasons, and scenario impact | 4 | 4 | 4 | 5 | 5 | **22** |
...
7. **4 → 3: Shareable circuit links removed.** URL serialization remains valuable future work, but a forecast receipt creates a complete sharing loop immediately without exposing a large encoded circuit payload.
...
### 3. Shareable forecast receipt

The result can be shared through the platform share sheet or copied to the clipboard. The receipt includes the leader, probability, modeled win count, the three explanatory signals, any strategy impact, and the product link.
